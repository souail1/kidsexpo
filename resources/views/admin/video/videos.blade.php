...
@section("content")
    ...
    <table id="videos" lay-filter="videotab"></table>
@endsection

@section("js")
    <script type="text/html" id="op">
        <a class="layui-btn layui-btn-xs edit_videos" lay-event="edit">
...
//                ,size: 'lg' // 'sm', 'lg'
            ...
            table.on('tool(videotab)', function(obj){
                var data = obj.data;      //获得当前行数据
                var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
                var tr = obj.tr;          //获得当前行 tr 的DOM对象
                if (layEvent == 'edit') {
                    dialog.open('编辑轮播图', '/admin/video/'+data.id+'/edit');
            ...
            // 排序
            table.on('sort(videotab)', function (obj) {
                var cond = $('.search_input').val();
                flushTable(cond, obj);
            });
            ...
            $('.add_btn').click(function () {
                dialog.open('添加轮播图', '/admin/video/create');
            });
